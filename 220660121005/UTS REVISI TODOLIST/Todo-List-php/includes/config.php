...
$conn = dbConnect();  // Menjalankan fungsi koneksi
...
function emailIsValid($email)
{
    ...
    if ($count > 0) {
        return true;  // Jika email ditemukan
    } else {
...
function createUser($email, $password)
{
    ...
    $result = mysqli_query($conn, $sql);
    return $result;  // Mengembalikan hasil query (berhasil atau tidak)
}
...
function textLimit($string, $limit)
{
    if (strlen($string) > $limit) {
        return substr($string, 0, $limit) . "...";  // Mengembalikan teks yang dipotong
    } else {
        return $string;  // Mengembalikan teks asli
    }
}
